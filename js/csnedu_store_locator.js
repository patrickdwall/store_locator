      ...
      for (var prop in itemFeatures) {
        ...
        if (itemFeatures[prop]) {
          switch(prop) {
            ...
            // pull marker icon from field
            case 'path':
              storeProps.icon = itemFeatures.path;
              break;

            case "gsl_props_misc":
              storeProps.misc = itemFeatures.gsl_props_misc;
              break;

            case "gsl_props_phone":
              storeProps.phone = itemFeatures.gsl_props_phone;
              break;

            case "gsl_props_web":
              var url = itemFeatures.gsl_props_web.split(',');
    ...
    attach: function (context, context_settings) {
      ...
      for (var mapid in Drupal.settings.gsl) {
        ...
        locator.map = new google.maps.Map(locator.elements.canvas, {
          ...
          minZoom: map_settings['mapzoom']
        });

        // set overlay
        if (map_settings['overlay']) {
          var imageBounds = new google.maps.LatLngBounds(
            new google.maps.LatLng(map_settings['overlayswlat'], map_settings['overlayswlon']),
            new google.maps.LatLng(map_settings['overlaynelat'], map_settings['overlaynelon'])
          );
          var overlaymap = new google.maps.GroundOverlay(map_settings['overlay'], imageBounds);
          overlaymap.setMap(locator.map);
        }
        ...
        locator.view.createMarker = function(store) {
          var customIcon = map_settings['marker_url'];
          if (store.getDetails().icon) {
            customIcon = store.getDetails().icon;
          }
